# title: "Gaussian process of 675V across Africa with parameter optimization"
...
outFldr <- "analysis/plots/prediction_simple_hyperparameter_optimization/"
...
dat <- read.csv("analysis/data/data_raw/validated_and_candidate_get_prevalence.csv")
admin0_africa <- readRDS("analysis/data/data_derived/sf_admin0_africa.rds")  %>%
  st_make_valid()
...
logit_admin0_plot <- ggplot() +
  geom_sf(data = admin0_africa, fill = NA, color = "black", linewidth = 0.3) +
  geom_point(data = dat %>% arrange(prevalence),
             aes(x = longitude, y = latitude, fill = plogis(z_est)),
             shape = 21, size = 1) +
  facet_wrap(~year) +
  scale_fill_viridis_c(option = "magma") +
  theme_bw()
...
for (i in 1:n_runs) {
  message("Running GP optimization ", i)

  # Random log-scale initial values for alpha, beta, lambda_space, lambda_time
  par_init <- log(runif(4, min = c(1, 0.1, 1, 200), max = c(10, 1, 20, 800^2)))

  lower <- log(c(0.5, 0.05, 0.5, 500))
  upper <- log(c(5, 10, 20, 400^2))

  opt_result <- GenSA(par = par_init,
                      fn = neg_log_likelihood,
                      lower = lower,
                      upper = upper,
                      control = list(max.call = 300),
                      dist_space = dist_space,
                      dist_time = dist_time,
                      z_est = dat$z_est)

  best_params <- exp(opt_result$par)

  results_list[[i]] <- tibble(
    run = i,
    alpha = best_params[1],
    beta = best_params[2],
    lambda_space = best_params[3],
    lambda_time = best_params[4],
    log_lik = opt_result$value
  )
}

# Combine into one DataFrame and pick best result
results_df <- bind_rows(results_list)
best_run <- results_df %>% arrange(log_lik) %>% slice(1)
...
# Compute the cross‐covariance between prediction grid and observed sites, but without adding noise on the diagonal (we want pure covariance)
K_pred <- get_GP_model(
...
# Perform GP prediction on empirical logit of observed prevalence. Predictions will be on real line, so transform these back to [0,1] range
...
# Calculate predictive variance; Solve: v = L \ (K_pred)
v <- forwardsolve(t(L), K_pred) # Lᵗ⁻¹ K_pred, dimensions: n x m
pred_var <- alpha_opt - colSums(v^2)
pred_var[pred_var < 0] <- 0 # for numerical stability
...
# Define thresholds for exceedance probability
logit_thresh <- qlogis(c(0.01, 0.05, 0.10))
names(logit_thresh) <- c("exceed_1", "exceed_5", "exceed_10")

# Compute exceedance probabilities
df_pred <- df_pred |>
  mutate(
    exceed_1  = 1 - pnorm(logit_thresh["exceed_1"], mean = z_pred, sd = z_sd),
    exceed_5  = 1 - pnorm(logit_thresh["exceed_5"], mean = z_pred, sd = z_sd),
    exceed_10 = 1 - pnorm(logit_thresh["exceed_10"], mean = z_pred, sd = z_sd),
  )

# Add mask or fade out areas with high uncertainty by adjusting the opacity (alpha) based on the relative uncertainty
df_pred <- df_pred %>%
  mutate(
    prevalence_sd = z_sd * p_pred * (1 - p_pred),
    relative_sd = prevalence_sd / p_pred,  # relative uncertainty
    alpha_mask = ifelse(relative_sd > 0.2, 0.2, 1.0)  # mask if >20% relative uncertainty
  )
...
prediction_plot_name = paste0(outFldr, "prevalence_prediction_plot_alpha", alpha_opt, "_beta", beta_opt, "_lambdaSpace", lambda_space_opt, "_lambdaTime", lambda_time_opt, ".png")
ggsave(prediction_plot_name, plot_predictions)

# Plot prevalence predictions grouped
grouped_pred <- ggplot(df_pred) +
  ...
  ggtitle(paste0("Estimated prevalence surface: alpha=", alpha_opt, " beta=", beta_opt, "\nlambdaSpace=", lambda_space_opt, " lambdaTime=", lambda_time_opt))
grouped_pred_name = paste0(outFldr, "prevalence_group_prediction_plot_alpha", alpha_opt, "_beta", beta_opt, "_lambdaSpace", lambda_space_opt, "_lambdaTime", lambda_time_opt, ".png")
ggsave(grouped_pred_name, grouped_pred)

# Plot posterior uncertainty by plotting standard variation
...
confidence_name = paste0(outFldr, "confidence_prediction_alpha", alpha_opt, "_beta", beta_opt, "_lambdaSpace", lambda_space_opt, "_lambdaTime", lambda_time_opt, ".png")
ggsave(confidence_name, confidence_plot)

# Plot predicted prevalence with alpha mapped to alpha_mask
plot_masked_plot <- ggplot(df_pred) +
  geom_sf(data = admin0_africa, fill = NA, color = "black", linewidth = 0.3) +
  geom_raster(aes(x = lon, y = lat, fill = p_pred, alpha = alpha_mask)) +
  facet_wrap(~year) +
  scale_fill_viridis_c(name = "Predicted Prevalence", limits = c(0, 1)) +
  scale_alpha_identity() +  # use precomputed alpha directly
  guides(alpha = FALSE) +   # remove alpha legend
  theme_bw()
plot_masked_name <- paste0(outFldr, "masked_prevalence_20perc_uncertainty_alpha", alpha_opt, "_beta", beta_opt, "_lambdaSpace", lambda_space_opt, "_lambdaTime", lambda_time_opt, ".png")
ggsave(plot_masked_name, plot_masked_plot)

# Plot the exceedance probability 1%
exceedance_prob_1perc_plot <- ggplot(df_pred) +
  geom_sf(data = admin0_africa, fill = NA, color = "black", linewidth = 0.3) +
  geom_raster(aes(x = lon, y = lat, fill = exceed_1)) +
  facet_wrap(~year) +
  scale_fill_viridis_c(name = "P(prevalence > 1%)") +
  theme_bw()
exceedance_prob_1perc_name = paste0(outFldr, "exceedance_prob_1perc_alpha", alpha_opt, "_beta", beta_opt, "_lambdaSpace", lambda_space_opt, "_lambdaTime", lambda_time_opt, ".png")
ggsave(exceedance_prob_1perc_name, exceedance_prob_1perc_plot)

# Plot the exceedance probability 5%
exceedance_prob_5perc_plot <- ggplot(df_pred) +
  geom_sf(data = admin0_africa, fill = NA, color = "black", linewidth = 0.3) +
  geom_raster(aes(x = lon, y = lat, fill = exceed_5)) +
  facet_wrap(~year) +
  scale_fill_viridis_c(name = "P(prevalence > 5%)") +
  theme_bw()
exceedance_prob_5perc_name = paste0(outFldr, "exceedance_prob_5perc_alpha", alpha_opt, "_beta", beta_opt, "_lambdaSpace", lambda_space_opt, "_lambdaTime", lambda_time_opt, ".png")
ggsave(exceedance_prob_5perc_name, exceedance_prob_5perc_plot)

# Plot the exceedance probability 10%
exceedance_prob_10perc_plot <- ggplot(df_pred) +
  geom_sf(data = admin0_africa, fill = NA, color = "black", linewidth = 0.3) +
  geom_raster(aes(x = lon, y = lat, fill = exceed_10)) +
  facet_wrap(~year) +
  scale_fill_viridis_c(name = "P(prevalence > 10%)") +
  theme_bw()
exceedance_prob_10perc_name = paste0(outFldr, "exceedance_prob_10perc_alpha", alpha_opt, "_beta", beta_opt, "_lambdaSpace", lambda_space_opt, "_lambdaTime", lambda_time_opt, ".png")
ggsave(exceedance_prob_10perc_name, exceedance_prob_10perc_plot)

# Save results to csv file
param_file <- file.path(outFldr, "gp_hyperparameters_k13-675-V.csv")
if (file.exists(param_file)) {
  existing <- read_csv(param_file, show_col_types = FALSE)
  results_df <- bind_rows(existing, results_df)
}
write_csv(results_df, param_file)
message("Saved all GP optimization results to: ", param_file)
